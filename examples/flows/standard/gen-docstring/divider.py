...
class Divider:
    ...
    def divide_file(cls, text) -> list[str]:
        ...
        splitted_content = []
        min_pos = matches[0].start() if matches else len(text)
        for i in range(len(matches)):
            start = matches[i].start()
            end = matches[i + 1].start() if i + 1 < len(matches) else len(text)
            splitted_content.append(text[start:end])
        if min_pos != 0:
            splitted_content.insert(0, text[:min_pos])
    ...
    def divide_half(cls, text) -> list[str]:
        ...
        if len(pos) > 1:  # Divide the code into two parts and every part start with a function.
            i = len(pos) // 2
            return [text[:pos[i][0]], text[pos[i][0]:]]
        if len(pos) == 1:  # Divide the code into two parts, [function define + body, other body].
            body = text[pos[0][1]:]
            body_lines = body.split('\n')
            body_ten_lines = '\n'.join(body_lines[:10])
            return [text[:pos[0][1]] + body_ten_lines, body[len(body_ten_lines):]]
        return [text]
    ...
    def merge_doc2code(cls, docstring: str, origin_code: str) -> str:
        ...
        pattern = r'""".*?"""'
        code = origin_code if len(funcs2) == 0 else origin_code[:pos2[0][0]]
        ...
        for i2 in range(len(funcs2)):  # add docstring for each function in origin_code
            ...
            try:
                ...
                code += part_full_code
                continue
            if new_doc := re.findall(
                pattern, docstring[pos1[i1][1] : pos1[i1 + 1][0]], re.DOTALL
            ):
                ...
                is_replace_doc = (
                    len(code_doc) > 0
                    and re.sub(r'\s+', '', func_body[: code_doc[0].start()]) == ''
                )
                if is_replace_doc:
